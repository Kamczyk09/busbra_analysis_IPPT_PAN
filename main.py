...
model = ResNet152.train_binary(nEpochs=30,
                              lr=1e-6,
                              model=model,
                              name="resnet152_busbra_raw")
ResNet152.evaluate_binary(name="resnet152_busbra_raw")
...
# model.load_state_dict(torch.load("models_checkpoints/resnet18_busbra_pretrained.pth")) #required model's name
# model.eval()
#
...
#
# for i in range(7):
#     image, mask, label = test_ds[100+i]
#     image_disp = denormalize(image, mean, std).float() / 255.0
#     sal = rise(model, image, true_label=label, res=200)
